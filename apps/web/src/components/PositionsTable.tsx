  ...
  if (positions.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center gap-3 py-20 text-center">
        ...
        <p className="text-sm text-zinc-500">Place a bet to get started</p>
        <Link
          to="/dashboard"
          className="mt-2 rounded-md bg-blue-500 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-400"
        >
          Browse Markets
        </Link>
      </div>
        ...
        <tbody>
          {positions.map((p) => (
            <tr key={p.id} className="border-b border-zinc-800/50">
              ...
              <td className="py-3 pr-4 text-zinc-400">{p.side}</td>
              <td className="py-3 pr-4 text-zinc-400">{p.quantity}</td>
              ...
              <td className="py-3 pr-4">
                <span className="rounded bg-yellow-500/20 px-1.5 py-0.5 text-xs font-semibold text-yellow-400">
                  OPEN
                </span>
              </td>
              <td className="py-3 text-zinc-500">{relativeTime(p.updatedAt)}</td>
            </tr>
          ))}
        </tbody>
      </table>
